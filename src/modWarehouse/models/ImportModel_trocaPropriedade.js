module.exports = function (app, cb) {
	...
	objSchema.shipment = {
		...
		columns: {
			...
			, 	CDSHIPME: Joi.string().label('Código do shipment')
	...
	objSchema.delivery = {
		...
		columns: {
			...
			, 	TPDELIVE: Joi.string().regex(/^(TFDT|LF|ZL1|YTFD)$/).required().label('Tipo da delivery')
	...
	objSchema.item = {
		...
		columns: {
			...
			, 	NRLOTE:   Joi.number().required().label('Lote numérico')
			, 	NRMAPA:   Joi.string().label('Lote MAPA')
			, 	DSALFANU: Joi.string().label('Lote Alfanumérico')
