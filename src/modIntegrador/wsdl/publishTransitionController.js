...
module.exports = function (app) {
  ...
  api.processarXML = async function (transition) {
    

    let o2x = require('object-to-xml');
    //console.log('chegou');
    console.log(o2x(transition));
    
     let caminho = process.cwd() + '/src/modIntegrador/wsdl/xmlTransition/';
    // Código usado para gravar o XML que vem da Neolog.
    let moment = require('moment');
    moment.locale('pt-BR');
    moment().format("DD/MM/YYYY HH:mm:ss");
    let nomeArquivo = caminho + moment().format("DD_MM_YYYY_HH_mm_ss") + "_" + transition.monitorableCode + ".xml";
    let xml = 
    `<SOAP-ENV:Envelope xmlns:SOAP-ENV="http://schemas.xmlsoap.org/soap/envelope/">
      <SOAP-ENV:Header/>
      <SOAP-ENV:Body>
        <ns3:publishTransition xmlns:ns2="http://www.neolog.com.br/cpl/publish/monitoring/occurrence/" xmlns:ns3="http://www.neolog.com.br/cpl/publish/monitoring/transition/">`
        + o2x(transition) +
    `   </ns3:publishTransition>
      </SOAP-ENV:Body>
     </SOAP-ENV:Envelope>`;
