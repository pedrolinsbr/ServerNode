...
module.exports = function (app, cb) {
  ...
  /**
   * @description Listar um dados da tabela G105.
   *
   * @async
   * @function api/listar
  ...
  api.listar = async function (req, res, next) {
    ...
    try {
      ...
      var [sqlWhere,sqlOrder,sqlPaginate,bindValues] = utils.retWherePagOrd(req.body,'G105',true);
      ...
      let result = await con.execute(
        {
          sql: ` 
           ...
           SELECT G105.IDG014,
                  ...
                  G105.SNDELETE,
                  G105.IDS001,
                  COUNT(G105.IdG105) OVER () as COUNT_LINHA
              FROM G105 G105
              JOIN G014 G014
                ON G014.IDG014 = G105.IDG014 `+
                    sqlWhere +
                    sqlOrder +
                    sqlPaginate,
          param: bindValues
  ...
  /**
   * @description Listar um dado na tabela G105.
   *
   * @async
   * @function api/buscar
   * @param {request} req - Possui as requisições para a função.
  ...
  api.buscar = async function (req, res, next) {
    ...
    try {
      ...
      var id = req.body.IDG105;
      logger.debug("Parametros buscar:", req.body);

      let result = await con.execute(
      {
        sql: ` SELECT G105.IDG014,
                      G014.DSOPERAC,
                      G105.TPTRANSP,
                      G105.DTCORTE,
                      G105.IDG105,
  ...
  /**
   * @description Salvar um dado na tabela G105.
   *
   * @async
   * @function api/salvar
   * @param {request} req - Possui as requisições para a função.
    ...
    try {
      ...
      let result = await con.insert({
        ...
        colunas: {
          ...
          DTCORTE : new Date(dateAux.getFullYear(), dateAux.getMonth(), 1, timeAux.hour, timeAux.minute,0,0),	
          SNDELETE: 0,
          DTCADAST: new Date(),
          IDS001  : req.UserId,
  ...
  /**
   * @description Atualizar um dado na tabela G105, .
   *
   * @async
  ...
  /**
   * @description Delete um dado na tabela G105.
   *
   * @async
