module.exports = function (app, cb) {
  ...
  var api = {};
  var dao = app.src.modDashboard.dao.TransferenciaDAO;

  var moment = require('moment');

  api.timeLineNf = async function (req, res, next) {
    await dao.timeLineNf(req, res, next)
      .then((result1) => {
        res.json(result1);
      })
      .catch((err) => {
  ...
  api.crossDockingTransferencia = async function (req, res, next) {
    await dao.crossDockingTransferencia(req, res, next)
      .then((result1) => {
        res.json(result1);
      })
      .catch((err) => {
  ...
  api.buscarTransfeDistrib = async function (req, res, next) {
    ...
    try {
      ...
      let transfeDistribEntregue = await dao.buscarTransfeDistrib(req, res, next);
      result.transfeDistrib = transfeDistribEntregue;

      req.body.aux = "AND G043.DTENTREG IS NULL"
      let transfeDistribTransito = await dao.buscarTransfeDistrib(req, res, next);
      for(let t of transfeDistribTransito ){
        result.transfeDistrib.push(t);
      }
      ...
      result.transfeDistrib.map(distrib => {
